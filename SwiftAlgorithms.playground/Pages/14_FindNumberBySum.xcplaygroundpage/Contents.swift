/*:
[<- Descending Order](@previous)
# 14 - Find Number By Sum
## Instrcutions:
 Write a function that takes an array of numbers (integers for the tests) and a target number. It should find two different items in the array that, when added together, give the target value. The indices of these items should then be returned in an array.

 The input will always be valid (numbers will be an array of length 2 or greater, and all of the items will be numbers; target will always be the sum of two different items from that array).
 
 Example:
 
 [1,2,3], 4 -> [0,2]
 
 [1234,5678,9012], 14690 -> [1,2]
 
 [2,2,3], 4 -> [0,1]
 
 [1,54,24,35], 59 -> [2,3]
 
 
[-> Persistent Bugger](@next)
*/

print(twosum(numbers: [1,2,3], target: 4))
print(twosum(numbers: [1234, 5678, 9012], target: 14690))
print(twosum(numbers: [2,2,3], target: 4))
print(twosum(numbers: [1,54,24,35], target: 59))
print("\nFind Number By Sum Tupple Algorithm")
print(findNumberBySum(numbers: [1,2,3], target: 4))
print(findNumberBySum(numbers: [1234, 5678, 9012], target: 14690))
print(findNumberBySum(numbers: [2,2,3], target: 4))
print(findNumberBySum(numbers: [1,54,24,35], target: 59))
